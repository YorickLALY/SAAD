# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 11:37:18 2021
...
signal = f.booleanScaler(signal, boolean_threshold)
...
angriff_rename_attack_1 = f.booleanScaler(angriff_rename_attack_1, boolean_threshold)
angriff_rename_attack_2 = f.booleanScaler(angriff_rename_attack_2, boolean_threshold)
...
pred_rename_attack_1 = [1.0 for i in range(990)] #Replay attack
for i  in range(113,990):
    pred_rename_attack_1[i] = 0.0
    
pred_rename_attack_2 = [1.0 for i in range(990)] #Replay attack
for i  in range(267,990):
    pred_rename_attack_2[i] = 0.0
...
angriff_replay_attack_1 = f.booleanScaler(angriff_replay_attack_1, boolean_threshold)
angriff_replay_attack_2 = f.booleanScaler(angriff_replay_attack_2, boolean_threshold)
...
pred_replay_attack_1 = [1.0 for i in range(990)] #Replay attack
for i  in range(395,990):
    pred_replay_attack_1[i] = 0.0
    
pred_replay_attack_2 = [1.0 for i in range(990)] #Replay attack
for i  in range(476,990):
    pred_replay_attack_2[i] = 0.0
...
angriff_sis_attack_1 = f.booleanScaler(angriff_sis_attack_1, boolean_threshold)
angriff_sis_attack_2 = f.booleanScaler(angriff_sis_attack_2, boolean_threshold)
...
pred_sis_attack = [0.0 for i in range(len(angriff_sis_attack_1[0]))] #no change in the behaviour of the process
...
angriff_fake_attack_1 = f.booleanScaler(angriff_fake_attack_1, boolean_threshold)
angriff_fake_attack_2 = f.booleanScaler(angriff_fake_attack_2, boolean_threshold)
...
pred_fake_attack = [1.0 for i in range(len(angriff_fake_attack_1[0]))] #Attack when Schranke goes down
pred_fake_attack = np.array(pred_fake_attack)
for i  in range(21*3,46*3):
    pred_fake_attack[i] = 0.0
...
if learning_on == 1:
    ...
    for epoch in range(10):
        ...
        for i, data in enumerate(trainloader,0):
            ...
            torch.save(net,'Autoencoder_pred_boolean.pt')
    ...
    plt.xlabel('Epoch')   
    plt.ylabel('Loss')
    plt.title('Evolution of the loss through the epoch for Autoencoder BooleanScaler')
    plt.show()
else:
    net = torch.load('Autoencoder_pred_boolean.pt')  #Load an existing network
...
threshold_classifier = threshold_90
