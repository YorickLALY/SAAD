# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 11:34:29 2021
...
train_data = StandardScaler().fit_transform(signal[:,0:(35000+window_size)])
test_data = StandardScaler().fit_transform(signal[:,35000:(len(signal[0]))])
...
angriff_rename_attack_1 = StandardScaler().fit_transform(angriff_rename_attack_1)
angriff_rename_attack_2 = StandardScaler().fit_transform(angriff_rename_attack_2)
...
angriff_replay_attack_1 = StandardScaler().fit_transform(angriff_replay_attack_1)
angriff_replay_attack_2 = StandardScaler().fit_transform(angriff_replay_attack_2)
...
angriff_sis_attack_1 = StandardScaler().fit_transform(angriff_sis_attack_1)
angriff_sis_attack_2 = StandardScaler().fit_transform(angriff_sis_attack_2)
...
angriff_fake_attack_1 = StandardScaler().fit_transform(angriff_fake_attack_1)
angriff_fake_attack_2 = StandardScaler().fit_transform(angriff_fake_attack_2)
...
if learning_on == 1:
    ...
    for epoch in range(30):
        ...
        for i, data in enumerate(trainloader,0):
            ...
            torch.save(net,'CNN_window_size_'+str(window_size)+'_StandardScaler.pt')
    ...
    plt.xlabel('Epoch')   
    plt.ylabel('Loss')
    plt.title('Evolution of the loss through the epoch for 1D-CNN StandardScaler window size = ' + str(window_size))
    plt.show()
else:
    net = torch.load('CNN_window_size_'+str(window_size)+'_StandardScaler_30epoch.pt')  #Load an existing network
