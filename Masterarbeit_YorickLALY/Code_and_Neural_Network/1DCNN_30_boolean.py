...
train_data = f.booleanScaler(signal[:,0:(35000+window_size)], boolean_threshold)
test_data = f.booleanScaler(signal[:,35000:(len(signal[0]))], boolean_threshold)
...
if learning_on == 1:
    ...
    for epoch in range(10):
        ...
        for i, data in enumerate(trainloader,0):
            ...
            torch.save(net,'CNN_window_size_'+str(window_size)+'_booleanScaler.pt')
    ...
    plt.xlabel('Epoch')   
    plt.ylabel('Loss')
    plt.title('Evolution of the loss through the epoch for 1D-CNN booleanScaler window size = ' + str(window_size))
    plt.show()
else:
    net = torch.load('CNN_window_size_'+str(window_size)+'_booleanScaler.pt')  #Load an existing network
